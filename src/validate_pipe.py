...
print('TRAIN SET SIZE PROPORTION:',
      round(train_df.shape[0]/all_data_len,3))
print('VAL SET SIZE PROPORTION:',
      round(val_df.shape[0]  /all_data_len,3)) 
if params["preprocess"]['TEST_START_PP']:
      print('TEST SET SIZE PROPORTION:',
            round(test_df.shape[0] /all_data_len,3))

window_size_timedelta = pd.Timedelta(value = params['featurize']['WINDOW_SIZE_PRO'], unit='W')
...
assert val_df.din_instante.iloc[0] + window_size_timedelta == pd.Timestamp(params['preprocess']['VAL_START_PP']), """
